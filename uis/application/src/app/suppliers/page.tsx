...
// Ruta: /suppliers (Application TrackFlow)
// ──────────────────────────────────────────────
...
import { PRODUCT_CATEGORY_OPTIONS, COUNTRY_OPTIONS } from "@/lib/types";
  ...
  if (state === "success" && suppliers.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="mt-2 text-gray-500">Los proveedores aparecerán aquí una vez registrados.</p>
        <Link href="/suppliers/new" className="mt-6 inline-block rounded-lg bg-tf-blue px-6 py-3 text-white hover:bg-tf-blue-dark transition-colors">Registrar proveedor</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/suppliers/new" className="rounded-lg bg-tf-blue px-4 py-2 text-sm text-white hover:bg-tf-blue-dark transition-colors">+ Nuevo proveedor</Link>
      </div>
      ...
      <div className="mb-6 flex flex-wrap items-end gap-4 rounded-lg border border-gray-200 bg-white p-4">
        <div>
          ...
          <select value={paisFilter} onChange={(e) => updateFilter("pais", e.target.value)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-tf-blue-light focus:outline-none">
        ...
        <div>
          ...
          <select value={categoriaFilter} onChange={(e) => updateFilter("categoria", e.target.value)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-tf-blue-light focus:outline-none">
            <option value="">Todas las categorías</option>
            {PRODUCT_CATEGORY_OPTIONS.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        {(paisFilter || categoriaFilter) && (
          <button onClick={clearFilters} className="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 transition-colors">Limpiar filtros</button>
        )}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12">
          ...
          <button onClick={clearFilters} className="mt-2 text-sm text-tf-blue hover:underline">Limpiar filtros</button>
        </div>
      ) : (
              ...
              {filtered.map((supplier) => (
                <tr key={supplier.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-medium text-tf-dark">
                    <Link href={`/suppliers/${supplier.id}`} className="hover:text-tf-blue-light">{supplier.nombre}</Link>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right font-mono text-gray-700">{supplier.tarifa.toFixed(2)}</td>
                  <td className="px-4 py-3 text-center"><SupplierBadge status={supplier.status} /></td>
                  <td className="px-4 py-3 text-right">
                    <Link href={`/suppliers/${supplier.id}`} className="text-sm text-tf-blue hover:underline">Detalle</Link>
                  </td>
                </tr>
              ))}
            </tbody>
