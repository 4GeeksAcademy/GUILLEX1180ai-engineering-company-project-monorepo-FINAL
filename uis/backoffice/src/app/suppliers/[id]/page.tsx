...
// Ruta: /suppliers/[id] (Backoffice TrackFlow)
// ──────────────────────────────────────────────
...
export default function SupplierDetailPage() {
  ...
  const handleRateSubmit = async (e: FormEvent) => {
    ...
    if (isNaN(tarifaNum) || tarifaNum <= 0) {
      setRateError("La tarifa debe ser un número estrictamente positivo");
      return;
    }
    setRateError(null);
    ...
    try {
      ...
      setRateError(err instanceof Error ? err.message : "Error al actualizar tarifa");
    } finally {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            ID: {supplier.id} · Última actualización:{" "}
            {new Date(supplier.updated_at).toLocaleString("es-ES")}
          </p>
        </div>
        <button
      ...
      <div className="mb-8 grid gap-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-tf-dark">
          Información general
        </h2>
      ...
      <div className="mb-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            {!editingRate ? (
              <p className="mt-1 text-2xl font-bold text-tf-blue">
              ...
              <form onSubmit={handleRateSubmit} className="mt-2 flex items-center gap-2">
                <div>
                  <input
                    ...
                    className="w-32 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-tf-blue-light focus:outline-none"
                    autoFocus
                  />
                  {rateError && (
                    <p className="mt-1 text-xs text-red-600">{rateError}</p>
                  )}
                </div>
                <button
                  type="submit"
          ...
          {!editingRate && (
            <button
              onClick={() => {
                setNewRate(supplier.tarifa.toString());
                setEditingRate(true);
              }}
