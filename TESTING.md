# TESTING — Estrategia de Pruebas (AUTH-088)
...
| Requerimiento | Módulo | Mínimo Requerido | Cobertura Actual |
|------|--------|-------------------|---------------|
| **AUTH-088** | Backend API completo (promedio) | **≥ 70%** | ✅ **96%** |
| **AUTH-088** | `routes/auth.py` — Autenticación | **≥ 70%** | ✅ **98%** |
| **API-042** | `routes/leads.py` — Leads | **≥ 60%** | ✅ **100%** |
| **API-042** | `routes/suppliers.py` — Proveedores | **≥ 60%** | ✅ **100%** |
| **API-042** | `routes/incidents.py` — Incidencias CSV | **≥ 60%** | ✅ **96%** |
| **API-042** | `routes/incidents_crud.py` — Incidencias CRUD | **≥ 60%** | ✅ **84%** |
| **API-042** | `incidents_core.py` — Motor de análisis | **≥ 60%** | ✅ **86%** |
| **API-042** | `incidents_db.py` — Persistencia | **≥ 60%** | ✅ **89%** |
| — | Tracker-core (TypeScript) | **≥ 60%** | ✅ **93.29% stmts / 88.97% branch** |

---

## 📊 Reporte de Cobertura — Backend (última ejecución)

Ejecutado con: `uv run pytest --cov=routes --cov=src --cov-report=term-missing -v`

```
Name                       Stmts   Miss  Cover   Missing
--------------------------------------------------------
routes/__init__.py             0      0   100%
routes/auth.py               104      2    98%   99, 226
routes/incidents.py           80      3    96%   83-84, 92
routes/incidents_crud.py      75     12    84%   90-97, 114-117, 229
routes/leads.py              135      0   100%
routes/suppliers.py           62      0   100%
src/api/__init__.py            2      2     0%   1-2
--------------------------------------------------------
TOTAL                        458     19    96%
```

> **Nota:** `src/api/__init__.py` al 0% es un archivo de inicialización vacío (solo `from .api import ...`), sin lógica ejecutable que probar. Todos los módulos funcionales superan el 84%, muy por encima del mínimo del 60% requerido por **API-042**.
...
> **Cobertura general de la API: 96%** — Los módulos `routes/auth.py` (98%) y todos los módulos de backoffice superan el 84%, cumpliendo holgadamente los umbrales de AUTH-088 (≥70%) y API-042 (≥60%).
